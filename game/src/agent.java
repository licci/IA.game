...
public class agent {
	...
	public void randomAttack(int startNode)
	{
	    ...
	    for(int x : availableStart){
	    	...
	    	if (x>0){
	    		ifSuccessAttack = action.attack(agentId,startNode,x,graph.map[startNode].numberOfSquares(),graph.map[startNode].numberOfCircles(),graph.map[startNode].numberOfTriangles());	
		    	/*
	...
	public void randomMove(int startNode)
	{
	    ...
	    for(int x : availableStart)
	    {
	    	if (x>0)
	    	{
	    		...
	    		ifSuccessMove =   action.move(agentId,startNode,x,graph.map[startNode].numberOfSquares(),graph.map[startNode].numberOfCircles(),graph.map[startNode].numberOfTriangles());	
	    		/*
